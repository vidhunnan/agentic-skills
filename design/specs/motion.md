...
**Source:** `website/app/globals.css` · `website/components/Reveal.tsx` · **Version:** `git:fcea6dd` (2026-08-19)
**Status:** built
...
Round 3 introduces a **scroll-linked highlight on the loop steps**: the active step
takes a `--blue` left border as it passes the middle of the viewport. It is
**reader-caused** — it tracks scroll position and does nothing on its own — so it
needs no second exception to the Layout rule above. It is listed here rather than in
the tables because it is not in `git:fcea6dd`; fold it in when round 3 ships and
re-pin the version.

The same applies to the rail's active-section marker, which is driven by the same
observer and is likewise a reader-caused state change, not an animation.
